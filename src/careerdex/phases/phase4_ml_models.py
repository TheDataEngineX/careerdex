...
class SalaryPredictor:
    ...
    def predict(
        ...
        if getattr(self, "_use_xgboost", False):
            ...
            # Simple heuristic fallback when xgboost is not available.
            base = self._BASE_SALARIES.get(title.lower(), 120_000)
            mult = self._LOCATION_MULTIPLIER.get(location.lower(), 1.0)
            delta = self._SENIORITY_DELTA.get(seniority.lower(), 0)
            p50 = (base + delta) * mult + years_exp * 1_500 + skill_count * 500
            p25 = max(30_000, p50 * 0.9)
            p75 = p50 * 1.1
            confidence = 0.55 + min(0.4, skill_count * 0.05)
            top_features = [
                f"title:{title.lower()}",
                f"location:{location.lower()}",
                f"seniority:{seniority.lower()}",
            ]
