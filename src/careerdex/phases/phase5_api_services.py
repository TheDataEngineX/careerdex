...
try:
    ...
    logger.warning("Salary predictor unavailable", error=str(exc))
    _salary = None
...
try:
    ...
    logger.warning("churn_predictor_unavailable", error=str(exc))
    _churn = None
...
async def predict_salary(
    ...
    if _salary is None:
        msg = "Salary prediction is disabled because xgboost is not installed."
        logger.error("salary_prediction_disabled", error=msg)
        return JSONResponse(
            status_code=503,
            content={"error": "missing_dependency", "message": msg},
        )
...
async def get_career_health(
    ...
    """
    if _churn is None:
        msg = "Churn prediction is disabled because scikit-learn is not installed."
        logger.error("churn_prediction_disabled", error=msg)
        return JSONResponse(
            status_code=503,
            content={"error": "missing_dependency", "message": msg},
        )
